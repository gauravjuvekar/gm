...
roi_histogram = cv2.calcHist([hsv_roi], [0], mask, [180], [0, 180])
cv2.normalize(roi_histogram, roi_histogram, 0, 255, cv2.NORM_MINMAX)
term_condition = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, 10, 1)
...
while True:
    ...
    dst  = cv2.calcBackProject([frame_hsv], [0], roi_histogram, [0, 180], 1)
    ret, armature_rects[0] = cv2.CamShift(dst, armature_rects[0], term_condition)
